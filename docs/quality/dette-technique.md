...
| # | Description | Feature | Fichier(s) | Impact |
|---|------------|---------|-----------|--------|
| N1 | **`BLOCK_MIN_AUDIENCE` sans mécanisme de rappel pour nouveaux blocs** : tout nouveau bloc de résultats omis de la table est visible pour tous (niveau novice par défaut). Signalé dans RETRO-web-experiments-01. | web/experiments | `apps/web/lib/audience/policy.ts` | Nouveau bloc visible pour novice sans validation pédagogique |
| N2 | **Téléchargement datasets sans support range requests** : le download de fichiers Parquet via Blob API ne supporte pas la reprise sur interruption. Signalé dans RETRO-web-datasets-02. | web/datasets, api/datasets | `apps/web/components/ibis/datasets/files-tab.tsx`, endpoint `/datasets/{id}/files/{file_id}/download` | Téléchargement de grands fichiers non résumable |
| N3 | **`test_index` dans `PreprocessResult` alourdit le joblib sur grands datasets** : pour un dataset >10 M lignes, la liste des indices test ajouterait plusieurs Mo à l'artefact. Signalé dans RETRO-api-ml-01. | api/ml, api/xai | `apps/api/ibis/modules/ml/preprocessing.py`, artefact `model.joblib` | Artefacts surdimensionnés sur très grands datasets |
...
| N5 | **Score éthique compute-on-read sans monitoring de latence** : `ethical_score()` est O(10) par dataset, acceptable aujourd'hui. Sans métrique de latence, le seuil critique (~10 000 datasets) sera atteint sans alerte. Signalé dans RETRO-api-datasets-02. | api/datasets | `apps/api/ibis/modules/datasets/service.py` (`to_card`, `to_detail`) | Dégradation silencieuse des performances du catalogue à grande échelle |
...
| N7 | **`AudienceWarning` sans test d'intégration** : le garde-fou visuel quand niveau effectif ≠ profil n'est pas couvert par les tests. Signalé dans RETRO-web-experiments-01. | web/experiments | `apps/web/components/ibis/audience/audience-warning.tsx` | Régression du garde-fou pédagogique non détectable |
| N8 | **Référence bibliographique Khelifi 2024 non vérifiable** : les 10 critères éthiques sont attribués à "Khelifi 2024" dans un commentaire source mais sans lien vers la publication. Impossible de vérifier la fidélité des critères à la source académique. Signalé dans RETRO-api-datasets-01. | api/datasets | `apps/api/ibis/modules/datasets/ethics.py` | Crédibilité académique non vérifiable |
