...
import { ApiAuthError, postLoginDestination, registerAction } from "@/lib/auth/session";
...
export default function RegisterPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    setErrorCode(null);
    try {
      const user = await registerAction(values.email, values.password);
  ...
  return (
    <GuestShell>
      <div className="text-center">
        <h2 className="mt-6 text-3xl font-bold">{t("registerTitle")}</h2>
      ...
      <form className="mt-8 space-y-6" onSubmit={form.handleSubmit(onSubmit)} noValidate>
        ...
        <Button type="submit" className="w-full" disabled={form.formState.isSubmitting}>
          {form.formState.isSubmitting ? t("creatingAccount") : t("signUp")}
        </Button>
      </form>
