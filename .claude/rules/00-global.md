...
> Valables sur tout projet, quelle que soit la stack. Fichier livré par Zelian Builder (projets neufs) ou copié par `/zelian:retro` — ne pas modifier au projet.
...
1. Lire les specs AVANT de toucher au code — **lecture CIBLÉE uniquement** : les documents du module concerné (résolus via Zelian Compass `.zelian/compass.json`, injectés automatiquement par les hooks v3), jamais l'ensemble de `docs/`. Protocole `/context` : DOC 2 §4.4 ; discipline détaillée : `07-context-discipline.md`
...
- ~~`/zelian:init`~~ — **déprécié (v3.0.0)** : les projets neufs se créent sur **Zelian Builder** (choix design + technos, projet téléchargé prêt avec ADRs, docs, marker et compass)
